...
def spawn_ardupilot_multirotor_node(
    pegasus_node_name: str = "ArduPilotMultirotor",
    graph_name: str = "ArduPilotMultirotorGraph",
    drone_prim: str = "/World/Quadrotor",
    usd_file: str = "~/.local/share/ov/data/documents/Kit/shared/exts/pegasus.simulator/pegasus/simulator/assets/Robots/Iris/iris.usd",
    robot_name: str = "robot_1",
    vehicle_id: int = 0,
    domain_id: int = 1,
    init_pos: list = None,
    init_orient: list = None,
    connection_type: str = "udpin",
    connection_ip: str = "127.0.0.1",
    ardupilot_autolaunch: bool = True,
    ardupilot_dir: str = "/isaac-sim/ardupilot",
    ardupilot_vehicle_model: str = "gazebo-iris",
    enable_lockstep: bool = False,
    num_rotors: int = 4,
    ...
    """
    Spawn an ArduPilot multirotor OmniGraph node (parallel to spawn_px4_multirotor_node).
    """
    if init_pos is None:
    ...
    graph_path = f"{drone_prim}/{graph_name}"
    ap_node_path = f"{graph_path}/{pegasus_node_name}"
    ...
    graph_handle, _, _, _ = og.Controller.edit(
        ...
        },
        {
            ...
            og.Controller.Keys.CREATE_NODES: [
                (domainIDReader_node_name, "omni.graph.core.ReadVariable"),
                (vehicleIDReader_node_name, "omni.graph.core.ReadVariable"),
                (pegasus_node_name, "pegasus.simulator.PegasusMultirotorArduPilotNode"),
                (getPrimPath_node_name, "omni.graph.nodes.GetPrimPath"),
                (playbackTick_node_name, "omni.graph.action.OnPlaybackTick"),
                (isaacReadSimTime_node_name, "isaacsim.core.nodes.IsaacReadSimulationTime"),
                (ros2Context_node_name, "isaacsim.ros2.bridge.ROS2Context"),
            ...
            og.Controller.Keys.SET_VALUES: [
                ...
                (f"{pegasus_node_name}.inputs:connectionType", connection_type),
                (f"{pegasus_node_name}.inputs:connectionIP", connection_ip),
                # rosDomainId must be int; ReadVariable outputs uchar — set explicitly (same value as graph variable)
                (f"{pegasus_node_name}.inputs:rosDomainId", int(domain_id)),
                (f"{pegasus_node_name}.inputs:ardupilotAutolaunch", ardupilot_autolaunch),
                (f"{pegasus_node_name}.inputs:ardupilotDir", ardupilot_dir),
                (f"{pegasus_node_name}.inputs:ardupilotVehicleModel", ardupilot_vehicle_model),
                (f"{pegasus_node_name}.inputs:enableLockstep", enable_lockstep),
                (f"{pegasus_node_name}.inputs:numRotors", num_rotors),
                (f"{pegasus_node_name}.inputs:updateRate", update_rate),
                (f"{pegasus_node_name}.inputs:inputOffset0", input_offset_0),
                ...
                (f"{pegasus_node_name}.inputs:zeroPositionArmed2", zero_position_armed_2),
                (f"{pegasus_node_name}.inputs:zeroPositionArmed3", zero_position_armed_3),
                (f"{pegasus_node_name}.inputs:inputMin", input_min),
                (f"{pegasus_node_name}.inputs:inputMax", input_max),
                (f"{pegasus_node_name}.inputs:fdmBasePort", fdm_base_port),
                (f"{getPrimPath_node_name}.inputs:prim", "../.."),
                (f"{domainIDReader_node_name}.inputs:variableName", domain_id_var_name),
                (f"{vehicleIDReader_node_name}.inputs:variableName", vehicle_id_var_name),
            ],
    ...
    print(f"ArduPilot node at {ap_node_path} created in on-demand graph.")
    return graph_handle